...
const Header = ({ email }: HeaderProps) => {
  ...
  const mailHref = `mailto:${email ?? "ronishprajapati0@gmail.com"}`;
            ...
            {!isScrolled && isHomePage ? (
              <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm animate-fade-in">
                <div className="absolute top-0 left-0 w-full h-full px-6 py-8 flex flex-col text-white">
                  ...
                  {/* CTA */}
                  <Button className="w-full mt-10 bg-white text-black hover:bg-white/90" asChild>
                    <a href={mailHref}>Hire Me</a>
                  </Button>
                </div>
              </div>
